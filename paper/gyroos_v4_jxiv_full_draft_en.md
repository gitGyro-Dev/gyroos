...
The implementation snapshot described in this paper is fixed by GitHub Release `v4.0.0` and archived on Zenodo as record `21641158`.

## 2. Theoretical Boundary

### 2.1 Structure, Slice, and Stability
...
### 2.2 Gyro Process and Operator Response
...
### 3.1 One Request, One Bounded Process
...
The current implementation uses an atomic SQLite-backed repository boundary. A complete Process result group is published atomically, followed by current-scope pointer update and immutable Process and Trajectory history retention. Typed reconstruction after restart preserves the recorded Runtime contract.
...
Exact workflow run identifiers and test counts should be included in the submission metadata appendix if required by the publication format. The paper's architectural claims do not depend on treating test count alone as evidence of theoretical validity.

## 9. Discussion

### 9.1 What Must Be Retained for Trajectory Continuity
...
### 9.2 Why Canonical Ownership Matters
...
### 9.3 Why Projection Remains Non-Canonical
...
### 9.4 Why Explicit References Matter
...
### 9.5 Why the Hierarchy Stops at W
...
Generative AI tools were used as research and development assistants for drafting, implementation support, structural review, and language refinement. The author reviewed and accepted responsibility for the definitions, architectural claims, implementation boundaries, code, and final manuscript content.
